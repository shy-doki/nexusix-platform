...
| v2.0.0 | 2026-06-09 | shy | 统一策略状态为五值枚举、修正路径格式为ID格式、修正field_operates为数组格式、补充组织架构/动态配置/消息通知/审计日志/行业适配模块功能需求 |
...
4. **字段级权限控制**：基于 JSONB 的 `field_operates` 设计，支持 QUERY/CREATE/UPDATE 三种访问类型的细粒度字段控制
...
- **关键字段**：
  ...
  - `path`：物化路径，斜杠分隔的ID路径格式，如 `/1/2/3/`，支持 LIKE 查询子树
...
2. 物化路径（path）在创建时自动生成，斜杠分隔的ID路径格式，如 `/1/2/3/`
...
| M05-F07 | 字段级权限 | 基于 field_operates 的 JSONB 字段控制，field_operates 为字段名数组，access_type 为独立字段 | P0 |
| M05-F08 | 策略状态管理 | 策略的五值状态控制：ACTIVE / DISABLED_SYSTEM_LEVEL / DISABLED_TENANT_LEVEL / DISABLED_ROLE_LEVEL / DISABLED_USER_LEVEL，DISABLED_* 状态标识策略在对应层级被禁用 | P0 |
...
| field_operates | JSONB | 允许操作的字段名数组，如 `["id", "user_name"]`，access_type 为独立字段已区分访问类型 |
| status | VARCHAR(20) | 策略状态：ACTIVE（生效）/ DISABLED_SYSTEM_LEVEL（系统级禁用）/ DISABLED_TENANT_LEVEL（租户级禁用）/ DISABLED_ROLE_LEVEL（角色级禁用）/ DISABLED_USER_LEVEL（用户级禁用） |
...
**拒绝优先原则**：高层级的 DISABLED_* 策略可覆盖低层级的 ACTIVE 策略，确保安全底线。
...
| 编号 | 功能 | 描述 | 优先级 |
|------|------|------|--------|
| M06-F01 | 菜单管理 | 系统菜单的树形管理，关联权限资源（sys_perm） | P0 |
| M06-F02 | 系统级字典 | 平台全局字典管理，所有租户共享 | P0 |
| M06-F03 | 租户级字典 | 租户自定义字典，仅本租户可见 | P0 |
| M06-F04 | 文件上传 | 文件上传和存储管理，支持本地/对象存储 | P0 |
| M06-F05 | 公告管理 | 系统公告的发布、撤回、置顶管理 | P1 |
| M06-F06 | 字典缓存 | 字典数据 Redis 缓存，变更时自动刷新 | P1 |
...
组织架构管理模块提供部门树、岗位和用户组管理，支持静态用户组和基于条件的动态用户组。部门管理支持无限层级，租户内数据隔离。

#### 功能列表

| 编号 | 功能 | 描述 | 优先级 |
|------|------|------|--------|
| M07-F01 | 部门管理 | 部门树的 CRUD，支持无限层级、租户内隔离，支持拖拽排序、层级调整 | P0 |
| M07-F02 | 岗位管理 | 岗位的定义和管理，关联到部门和用户 | P0 |
| M07-F03 | 静态用户组 | 手动添加/移除成员的用户组，适用于固定团队 | P1 |
| M07-F04 | 动态用户组 | 基于条件规则（部门、岗位、标签等）自动匹配成员的用户组 | P1 |
| M07-F05 | 用户组权限 | 用户组可作为权限策略的分配单位 | P1 |
| M07-F06 | 组织架构图 | 可视化展示组织架构树形图 | P2 |

#### 业务规则

1. 部门树采用与租户树相同的 parent_id + path 物化路径设计，支持无限层级
2. 部门数据租户内隔离，不同租户的部门树互不可见
3. 动态用户组的成员在每次查询时实时计算
4. 用户可同时属于多个用户组
5. 岗位变更需同步更新用户-租户关联中的 dept_id
...
动态配置管理提供运行时可配置的表单、数据源和打印模板能力，降低硬编码依赖，提升系统灵活性。

#### 功能列表

| 编号 | 功能 | 描述 | 优先级 |
|------|------|------|--------|
| M08-F01 | 动态表单配置 | 基于 JSON Schema 定义表单结构，运行时渲染和校验 | P1 |
| M08-F02 | 动态数据源-业务表 | 配置业务表作为数据源，支持字段映射和条件过滤 | P1 |
| M08-F03 | 动态数据源-API | 配置外部 API 作为数据源，支持请求参数映射和响应转换 | P1 |
| M08-F04 | 动态数据源-字典 | 配置字典项作为数据源，用于下拉选择等场景 | P1 |
| M08-F05 | 动态数据源-SQL | 配置自定义 SQL 查询作为数据源，需安全审查 | P2 |
| M08-F06 | 打印模板配置 | 基于 HTML/CSS 的打印模板设计，支持变量绑定和条件渲染 | P2 |
...
消息通知中心提供消息模板管理、站内信收发和定时消息任务功能，支持单次、周期和事件触发三种消息调度模式。

#### 功能列表

| 编号 | 功能 | 描述 | 优先级 |
|------|------|------|--------|
| M09-F01 | 消息模板管理 | 定义消息模板，支持变量占位符（如 `${userName}`） | P1 |
| M09-F02 | 站内信收发 | 系统内消息的发送、接收、已读/未读管理 | P0 |
| M09-F03 | 定时消息-单次 | 指定时间点发送一次的消息 | P1 |
| M09-F04 | 定时消息-周期 | 按 Cron 表达式周期性发送的消息 | P1 |
| M09-F05 | 定时消息-事件 | 由业务事件（如租户过期、配额告警）触发的消息 | P1 |
| M09-F06 | 消息免打扰 | 用户可设置免打扰时段，期间不推送通知 | P2 |
...
审计日志模块提供操作日志、登录日志和数据变更审计功能，确保系统操作可追溯、数据变更可审计。

#### 功能列表

| 编号 | 功能 | 描述 | 优先级 |
|------|------|------|--------|
| M10-F01 | 操作日志 | 记录用户的关键操作，基于 @OperationLog 注解自动采集 | P0 |
| M10-F02 | 登录日志 | 记录所有登录/登出事件，包含 IP、设备、时间等信息 | P0 |
| M10-F03 | 数据变更审计 | 记录业务数据的变更前后快照，支持字段级差异对比 | P1 |

#### 业务规则

1. 操作日志不可修改和删除，保留期限不少于180天
2. 登录日志记录成功和失败的登录尝试，支持按时间/用户/IP筛选
3. 数据变更审计通过 MyBatis-Plus 拦截器自动采集，记录变更前后快照
...
行业适配模块提供行业模板管理、表单模板管理和租户个性化配置能力，支持不同行业场景的快速适配和租户级别的个性化定制。

#### 功能列表

| 编号 | 功能 | 描述 | 优先级 |
|------|------|------|--------|
| M11-F01 | 行业模板管理 | 定义和管理行业模板（如制造、零售、金融等），包含预置菜单、权限、表单配置 | P1 |
| M11-F02 | 表单模板管理 | 管理行业预置的表单模板，支持基于模板快速创建业务表单 | P1 |
| M11-F03 | 租户个性化配置 | 租户基于行业模板进行个性化定制，包括字段扩展、流程调整、界面风格等 | P1 |

#### 业务规则

1. 行业模板由超级管理员维护，租户可基于模板创建个性化配置
2. 租户个性化配置不影响行业模板本身，配置独立存储
3. 行业模板变更时，已基于该模板创建的租户配置不受影响
4. 表单模板与动态配置中心（M08）联动，模板中的表单配置可直接导入动态表单
...
7. **策略状态分类**：
   - 遍历策略列表，按 `status` 字段分为：
     ...
     - `DISABLED_SYSTEM_LEVEL` / `DISABLED_TENANT_LEVEL` / `DISABLED_ROLE_LEVEL` / `DISABLED_USER_LEVEL` 策略的 perm_id → disabledPolicyPermIdSet
8. **提取权限编码**：
   - allPermCodeList：所有关联的权限编码
   - validPermCodeList：ACTIVE 策略对应的权限编码（允许）
   - invalidPermCodeList：DISABLED_* 策略对应的权限编码（拒绝）
9. **四层策略合并**（按优先级从低到高）：
...
10. **拒绝优先原则**：高层级的 DISABLED_* 策略覆盖低层级的 ACTIVE 策略
...
3. 按 `table_name` 分组，每个表构建 `EntityFieldPerm` 对象：
   ...
   - `invisibleFields`：DISABLED_* 策略的 field_operates 字段合并
...
```json
// field_operates（允许操作的字段名数组，access_type 为独立字段已区分访问类型）
["id", "user_name", "nick_name", "email", "phone"]
```

**涉及表**：`sys_perm_policy`（field_operates JSONB 字段）
...
| 编号 | 用户故事 |
|------|----------|
| M06-US01 | 作为超级管理员，我希望能够管理系统菜单并关联权限资源，以便控制用户可见的功能入口 |
| M06-US02 | 作为租户管理员，我希望能够维护租户级字典，以便满足本租户特有的数据分类需求 |
| M06-US03 | 作为普通员工，我希望能够上传文件并受配额控制，以便在允许范围内管理工作文件 |

### M07 组织架构管理

| 编号 | 用户故事 |
|------|----------|
| M07-US01 | 作为租户管理员，我希望能够通过拖拽调整部门层级，以便快速适应组织架构调整 |
| M07-US02 | 作为租户管理员，我希望能够创建动态用户组，以便自动将符合条件的用户纳入管理 |
| M07-US03 | 作为普通员工，我希望能够查看组织架构图，以便了解公司结构和汇报关系 |

### M08 动态配置管理

| 编号 | 用户故事 |
|------|----------|
| M08-US01 | 作为租户管理员，我希望能够通过 JSON Schema 配置业务表单，以便无需开发即可新增业务表单 |
| M08-US02 | 作为租户管理员，我希望能够配置外部 API 作为数据源，以便在表单中引用外部系统数据 |
| M08-US03 | 作为超级管理员，我希望能够设计打印模板，以便不同租户可以使用自定义的打印格式 |

### M09 消息通知中心

| 编号 | 用户故事 |
|------|----------|
| M09-US01 | 作为普通员工，我希望能够收到站内信通知，以便及时了解系统动态和待办事项 |
| M09-US02 | 作为租户管理员，我希望能够配置事件触发消息，以便在租户过期或配额告警时自动通知相关人员 |
| M09-US03 | 作为普通员工，我希望能够设置免打扰时段，以便在非工作时间不被打扰 |

### M10 审计日志

| 编号 | 用户故事 |
|------|----------|
| M10-US01 | 作为审计员，我希望能够查看操作日志并按时间/用户/操作类型筛选，以便追踪可疑行为 |
| M10-US02 | 作为审计员，我希望能够查看数据变更前后的差异对比，以便确认数据是否被篡改 |
| M10-US03 | 作为审计员，我希望能够查看登录日志并分析异常登录行为，以便及时发现安全威胁 |

### M11 行业适配

| 编号 | 用户故事 |
|------|----------|
| M11-US01 | 作为超级管理员，我希望能够定义行业模板，以便新租户开通时快速适配行业场景 |
| M11-US02 | 作为租户管理员，我希望能够基于行业模板进行个性化配置，以便满足本企业的特殊需求 |
| M11-US03 | 作为租户管理员，我希望能够使用行业预置的表单模板，以便快速搭建业务表单 |
...
flowchart TD
    ...
    E --> F[计算物化路径 path<br/>斜杠分隔的ID路径格式，如 /1/2/3/]
...
flowchart TD
    ...
    E5 --> E6[批量查询 sys_perm]
    E6 --> E7[策略状态分类<br/>ACTIVE / DISABLED_*]
    E7 --> E8[四层策略合并<br/>系统→租户→角色→用户]
    E8 --> E9[拒绝优先原则<br/>DISABLED_*覆盖ACTIVE]
    E9 --> F[第5层：字段控制层]
    E3 --> G[登录成功]
...
flowchart TD
    ...
    E -->|DISABLED_*| G[添加到拒绝权限集<br/>invalidPermCodeList]
...
- **中栏（40%宽度）**：
  - 选中目标的策略列表
  - 每条策略显示：策略名称、关联权限、状态标签（ACTIVE 绿色/DISABLED_* 红色）
  - 新增策略按钮
...
- **右栏（35%宽度）**：
  - 策略详情编辑表单：
    ...
    - 字段权限配置：
      - 左侧：表所有字段列表（可勾选）
      - 右侧：已选字段列表（field_operates）
    - 策略状态：ACTIVE / DISABLED_SYSTEM_LEVEL / DISABLED_TENANT_LEVEL / DISABLED_ROLE_LEVEL / DISABLED_USER_LEVEL 切换
...
| PermPolicyStatus | DISABLED_SYSTEM_LEVEL | 系统级禁用 |
| PermPolicyStatus | DISABLED_TENANT_LEVEL | 租户级禁用 |
| PermPolicyStatus | DISABLED_ROLE_LEVEL | 角色级禁用 |
| PermPolicyStatus | DISABLED_USER_LEVEL | 用户级禁用 |
...
| 序号 | 表名 | 说明 |
|------|------|------|
| 1 | sys_tenant | 租户主表，存储租户基本信息、层级路径、状态等 |
| 2 | sys_tenant_quota | 租户配额表，存储各项资源的配额与使用量 |

#### M02 计费与套餐中心（2张）

| 序号 | 表名 | 说明 |
|------|------|------|
| 3 | sys_plan | 套餐定义表，存储套餐产品信息、计费周期、价格 |
| 4 | sys_plan_quota_template | 套餐配额模板表，定义套餐对应的各项资源配额 |

#### M03 配额管理（2张）

| 序号 | 表名 | 说明 |
|------|------|------|
| 5 | sys_tenant_subscription | 租户订阅表，记录订阅类型、来源、时间范围、状态 |
| 6 | sys_quota_adjustment | 配额调整记录表，记录加油包、补偿、惩罚等调整 |

#### M04 IAM身份认证（3张）

| 序号 | 表名 | 说明 |
|------|------|------|
| 7 | sys_user | 用户主表，全局唯一，存储用户名、密码哈希、状态等 |
| 8 | sys_user_tenant_rel | 用户-租户关联表，维护多租户绑定、管理员标识、默认租户 |
| 9 | sys_user_token | 用户Token记录表，记录登录设备、Token有效期等 |

#### M05 权限控制中心（6张）

| 序号 | 表名 | 说明 |
|------|------|------|
| 10 | sys_perm | 权限资源表，树形结构，支持菜单/按钮/API/数据类型 |
| 11 | sys_perm_policy | 权限策略表，核心策略控制，含字段级权限（field_operates） |
| 12 | sys_user_perm_rel | 用户权限关联表，用户与策略的直接关联 |
| 13 | sys_role | 角色表，定义角色编码、描述、数据范围 |
| 14 | sys_role_policy | 角色策略关联表，角色与策略的绑定关系 |
| 15 | sys_user_role_rel | 用户角色关联表，用户与角色的绑定关系 |

#### M06 系统支撑服务（5张）

| 序号 | 表名 | 说明 |
|------|------|------|
| 16 | sys_menu | 菜单表，树形结构，关联权限资源 |
| 17 | sys_dict_type | 字典类型表，区分系统级/租户级字典 |
| 18 | sys_dict_data | 字典数据表，存储字典项键值对 |
| 19 | sys_file | 文件表，记录文件上传信息、存储路径、大小 |
| 20 | sys_notice | 公告表，存储公告内容、发布范围、状态 |

#### M07 组织架构管理（4张）

| 序号 | 表名 | 说明 |
|------|------|------|
| 21 | sys_dept | 部门表，树形结构，租户内隔离，支持无限层级 |
| 22 | sys_post | 岗位表，定义岗位编码、名称、排序 |
| 23 | sys_user_group | 用户组表，区分静态/动态用户组 |
| 24 | sys_user_group_rel | 用户组成员关联表，静态组手动关联成员 |

#### M08 动态配置管理（4张）

| 序号 | 表名 | 说明 |
|------|------|------|
| 25 | sys_dynamic_form | 动态表单表，存储 JSON Schema 定义 |
| 26 | sys_dynamic_datasource | 动态数据源表，配置业务表/API/字典/SQL数据源 |
| 27 | sys_print_template | 打印模板表，存储 HTML/CSS 模板及变量绑定 |
| 28 | sys_dynamic_form_data | 动态表单数据表，存储表单提交的 JSON 数据 |

#### M09 消息通知中心（4张）

| 序号 | 表名 | 说明 |
|------|------|------|
| 29 | sys_message_template | 消息模板表，定义模板内容、变量占位符 |
| 30 | sys_message | 站内信表，存储消息内容、发送者、接收者、已读状态 |
| 31 | sys_scheduled_message | 定时消息任务表，配置单次/周期/事件触发消息 |
| 32 | sys_message_read | 消息已读记录表，记录用户的消息阅读状态 |

#### M10 审计日志（3张）

| 序号 | 表名 | 说明 |
|------|------|------|
| 33 | sys_operation_log | 操作日志表，记录用户关键操作、请求参数、响应结果 |
| 34 | sys_login_log | 登录日志表，记录登录/登出事件、IP、设备信息 |
| 35 | sys_data_audit_log | 数据变更审计表，记录变更前后快照、字段级差异 |

#### M11 行业适配（6张）

| 序号 | 表名 | 说明 |
|------|------|------|
| 36 | sys_industry_template | 行业模板表，定义行业类型、预置菜单和权限配置 |
| 37 | sys_industry_form_template | 行业表单模板表，行业预置的表单 JSON Schema |
| 38 | sys_tenant_custom_config | 租户个性化配置表，存储租户基于模板的定制配置 |
| 39 | sys_industry_template_menu | 行业模板菜单关联表，行业模板预置的菜单配置 |
| 40 | sys_industry_template_perm | 行业模板权限关联表，行业模板预置的权限策略 |
| 41 | sys_tenant_config_field | 租户配置字段扩展表，存储租户个性化字段扩展定义 |
