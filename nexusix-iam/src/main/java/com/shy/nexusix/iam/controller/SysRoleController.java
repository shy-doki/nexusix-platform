...
public class SysRoleController {
    ...
    @Operation(summary = "查询角色已分配的权限列表", description = "返回指定角色关联的权限树形结构")
    public ApiResponse queryRolePermissions(@PathVariable Long roleId) {
        List<SysPermissionTreeVO> permissionTree = iSysPermissionPolicyService.queryPermissionsByRoleId(roleId);
    ...
    @Operation(summary = "分配角色用户", description = "为指定角色分配用户，先清后写")
    public ApiResponse assignRoleUsers(@Valid @RequestBody SysRoleUserAssignRTO assignParam) {
        Integer affectedRows = iSysRoleService.assignRoleUsers(assignParam);
        return ApiResponse.success(affectedRows);
    }
